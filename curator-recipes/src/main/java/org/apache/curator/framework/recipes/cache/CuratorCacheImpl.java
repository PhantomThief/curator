...
class CuratorCacheImpl implements CuratorCache, CuratorCacheBridge
{
    ...
    private final Function<Runnable, CompletableFuture<Void>> runSafeProc;
    ...
    CuratorCacheImpl(CuratorFramework client, CuratorCacheStorage storage, String path, Options[] optionsArg, Consumer<Exception> exceptionHandler, Function<Runnable, CompletableFuture<Void>> runSafeProc)
    {
        ...
        this.runSafeProc = (runSafeProc != null) ? runSafeProc : client::runSafe;
    }
    ...
    private void callListeners(Consumer<CuratorCacheListener> proc)
    {
        if ( state.get() == State.STARTED )
        {
            runSafeProc.apply(() -> listenerManager.forEach(proc));
        }
    }
