    ...
    public void testTreeCache() throws Exception    // copied from TestTreeCache#testBasics()
    {
        ...
        try (CuratorFramework client = CuratorFrameworkFactory.newClient(server.getConnectString(), timing.session(), timing.connection(), new RetryOneTime(1)))
        {
            ...
            try (CuratorCache cache = CuratorCache.build(client, "/test"))
            {
                ...
                Assert.assertEquals(toMap(cache.streamImmediateChildren("/test")).keySet(), ImmutableSet.of());
                Assert.assertEquals(cache.streamImmediateChildren("/t").count(), 0);
                Assert.assertEquals(cache.streamImmediateChildren("/testing").count(), 0);

                client.create().forPath("/test/one", "hey there".getBytes());
                treeCacheBase.assertEvent(TreeCacheEvent.Type.NODE_ADDED, "/test/one");
                Assert.assertEquals(toMap(cache.streamImmediateChildren("/test")).keySet(), ImmutableSet.of("/test/one"));
                Assert.assertEquals(new String(cache.get("/test/one").orElseThrow(AssertionError::new).getData()), "hey there");
                Assert.assertEquals(toMap(cache.streamImmediateChildren("/test/one")).keySet(), ImmutableSet.of());
                Assert.assertEquals(cache.streamImmediateChildren("/test/o").count(), 0);
                Assert.assertEquals(cache.streamImmediateChildren("/test/onely").count(), 0);
